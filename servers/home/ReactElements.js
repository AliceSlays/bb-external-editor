/* React element library for Bitburner - v0.5.3

	v0.5.0 - Initial release	(Note: Future changes will likely break some things.)
	v0.5.1 - Earlier release	(Breaking changes: Fixed missing ID parameters in `rLinkCL`, `rLinkFunc`, and `rLinkSC`.
								 Added: `rBlue`, `rOrange`, and `rPurple`.
								 Minor improvements: `runCommands`.)
	v0.5.2 - Earlier release	(Reduced RAM usage: Switched from `ns.getHostname()` to `ns.self().server`.
								 Minor improvements: `clone` [you can now list properties to ignore; helps avoid triggering depreciated methods].)
	v0.5.3 - Current release	(Fix: Corrected the way that custom styles passed to the methods here are handled.
								 Minor fix: Changed some "==" and "!=" to "===" and "!==", respectively.
								 Minor improvements: Removed some unnecessary conditional checks.)
*/
...
/**
 * clone: Makes a unique copy of an object, not including functions.  Optionally, you can remove properties from the object as well.
 * 		  Non-objects (Booleans, numbers, strings, etc...) passed to the function are returned as-is.
 *
 * @param	{any}		obj				The object to be copied.
 * @param	{string[]=}	ignoredProps	(Optional) Names of properties to skip when cloning.
 * @returns	{any}						The copy of the object.
 **/
export function clone (obj, ignoredProps = []) {
	...
	if (!isObject(obj)) {  // Return non-objects unmodified.
		return obj;
	}
	...
	if (isArray(obj)) {
		...
		for (let i = 0; i < obj.length; i++) {
			arrResult.push(clone(obj[i], ignoredProps));
		}
		return arrResult;  // Cloned array.
	}
	if (isSet(obj)) {
		...
		return setResult;  // Cloned set.
	}
	if (isMap(obj)) {
		result = new Map();
	}
	for (let key of keys) {
		if (!ignoredProps.includes(key) && typeof obj[key] !== "function") {  // Ignore ignored properties and functions.
			if (typeof obj[key] !== "object") {
				result[key] = obj[key];  // Add non-objects to the object.  (Booleans, numbers, strings, etc...)
			} else if (isArray(obj[key])) {  // Handle arrays.
				let tempArr = [];
				for (let value of obj[key]) {
					tempArr.push(clone(value, ignoredProps));
				}
				result[key] = tempArr;  // Add a copy of the array.
			} else if (isSet(obj[key])) {  // Handle sets.
				let tempSet = new Set();
				for (let value of obj[key]) {
					tempSet.add(clone(value, ignoredProps));
				}
				result[key] = tempSet;  // Add a copy of the set.
			} else if (isMap(obj[key])) {  // Handle maps.
				let tempMap = new Map();
				for (let value of obj[key]) {
					tempMap.set(clone(value[0], ignoredProps), clone(value[1], ignoredProps));
				}
				result[key] = tempMap;  // Add a copy of the map.
			} else {  // Handle other objects.
				result[key] = clone(obj[key]);  // Add sub-objects too.
			}
		}
	}
	return result;
...
/**
 * runCommands: Asynchronously runs the given string as a command and then returns once that command has finished running.
 * 	!! **NOTE: This is meant for commands which run quickly and then end, not for background processes which continue to run.** !!
 *
...
export async function runCommands (ns, commands) {  // deepscan-ignore-line
	...
	while (ns.fileExists(fname, "home") || ns.fileExists(fname, ns.self().server)) {  // Make sure that the file doesn't already exist.
		fname = "temp_" + getRandomHexString() + ".js";
	}
	ns.write(fname, script);  // Create the temporary file.
	if (ns.self().server != "home") {
		ns.mv(fname, ns.self().server, "home");
	}
	let pid = ns.exec(fname, "home");
	while (pid == 0) {  // Run the script.
		await ns.asleep(50);
		pid = ns.exec(fname, "home");
	}
	await ns.asleep(100);
	while (ns.isRunning(pid)) {  // Wait for the script to finish running.
		await ns.asleep(100);
	}
 ...
 **/
export function rAudio (source, id = "", style, altText = "") {
 ...
 * @returns	{React.ReactNode}
 **/
export function rButton (text, trigger, id = "", style, altText = "") {
 ...
 * @returns	{React.ReactNode}
 **/
export function rCheckbox (label, trigger, checked = false, id = "", style, altText = "") {
...
export function rCheckboxValue (id) {
	if (doc.getElementById(id) !== null && "checked" in doc.getElementById(id)) {
 ...
 * @returns	{React.ReactNode}
 **/
export function rDropdown (dropdownItems, trigger, label, defaultIndex = 0, style, id = "") {
 ...
 * @returns	{Element|null|undefined}	Either the value of the currently selected element or `undefined` if not found.
 **/
 ...
 * @returns	{React.ReactNode}
 **/
export function rLinkCL (text, command, style, id = "", altText = "") {
 ...
 * @returns	{React.ReactNode}
 **/
export function rLinkFunc (text, func, style, id = "", altText = "") {
	let linkStyle = clone(defaultStyle);
	if (style !== undefined) {
		linkStyle = Object.assign(linkStyle, style);  // Merge the style parameter's values into the default styling.
	}
 ...
 * @returns	{React.ReactNode}
 **/
export function rLinkSC (text, command, ns, style, id = "", altText = "") {
	let linkStyle = clone(defaultStyle);
	if (style !== undefined) {
		linkStyle = Object.assign(linkStyle, style);  // Merge the style parameter's values into the default styling.
	}
 ...
 * @returns	{React.ReactNode}
 **/
export function rText (text, style, id = "", altText = "") {
...
// Text colors:
export function rBlue (text) {
	return rText(text, { color: "#00f" });
}
...
export function rOrange (text) {
	return rText(text, { color: "#f60" });
}

export function rPurple (text) {
	return rText(text, { color: "#808" });
}
